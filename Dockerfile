...
FROM node:22.22.1-bookworm-slim@sha256:4f77a690f2f8946ab16fe1e791a3ac0667ae1c3575c3e4d0d4589e9ed5bfaf3d AS gateway-runtime
ENV NODE_ENV=production
WORKDIR /app
COPY --chown=node:node --chmod=0555 docker-gateway.js ./docker-gateway.js
USER node
EXPOSE 8080
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD ["node", "-e", "fetch('http://127.0.0.1:8080/healthz',{signal:AbortSignal.timeout(2000)}).then(r=>{if(!r.ok)process.exit(1)}).catch(()=>process.exit(1))"]
CMD ["node", "docker-gateway.js"]
...
COPY --chown=node:node --chmod=0555 server.js ./server.js
COPY --chown=node:node --chmod=0555 ops-tools.js ./ops-tools.js
COPY --chown=node:node --chmod=0444 package.json ./package.json
USER node
EXPOSE 3000
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
...
FROM node:22.22.1-bookworm-slim@sha256:4f77a690f2f8946ab16fe1e791a3ac0667ae1c3575c3e4d0d4589e9ed5bfaf3d AS mcp-ops-runtime
ENV NODE_ENV=production
RUN apt-get update \
